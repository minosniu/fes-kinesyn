function PlotOnOffSeparate(SN_type,subn,type,task,root,Dir,Axisfile)
...
N = length(ExpList);
Figname = [root '//Figures//OnOffSeparate//' subn '_' type '_' task '_'];
load(Axisfile);
...
hold on;
for i = 1:N
    plot(Period{i}.Time,0-Period{i}.Vel_filt_elbow,'Color',TraColor(SN_type,:),'LineWidth',1);
end
hold on;
plot(Period{N_longest}.Time,0-Vel_elbow_ave,'Color',VelColor(SN_type,:),'LineWidth',3);
...
hold on;
for i = 1:N
    plot(Period{i}.Time,Period{i}.Vel_filt_hand_xy,'Color',TraColor(SN_type,:),'LineWidth',1);
end
hold on;
plot(Period{N_longest}.Time,Vel_hand_ave,'Color',VelColor(SN_type,:),'LineWidth',3);
